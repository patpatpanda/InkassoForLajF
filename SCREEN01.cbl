       ...
       SCREEN-LOOP.
           ...
           DISPLAY "Välj: (N = Next, P = Previous, Q = End)" AT
             LINE 19 COL 10.
           ACCEPT USER-INPUT.
           ...
           PERFORM VARYING LINE-INX FROM LINE-INX BY 1 UNTIL LINE-INX >
             (SCREEN-NUMBER * 10)
               DISPLAY CSV-SCREEN-FIRST-NAME(LINE-INX)
                 ...
                 2
                 WITH FOREGROUND-COLOR 2 *> Grön text
               DISPLAY CSV-SCREEN-LAST-NAME(LINE-INX)
                 AT LINE (LINE-INX - ((SCREEN-NUMBER - 1) * 10)) + 5 COL
                 24
                 WITH FOREGROUND-COLOR 1 *> Blå text
               DISPLAY CSV-SCREEN-DATEOFBIRTH(LINE-INX)
                 AT LINE (LINE-INX - ((SCREEN-NUMBER - 1) * 10)) + 5 COL
                 46
                 WITH FOREGROUND-COLOR 4 *> Magenta text
               DISPLAY CSV-SCREEN-FEE(LINE-INX)
                 AT LINE (LINE-INX - ((SCREEN-NUMBER - 1) * 10)) + 5 COL
                 56
                 WITH FOREGROUND-COLOR 3 *> Röd text
           END-PERFORM.
